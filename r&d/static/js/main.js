...
const screenly = {
  settings: {
    ...
    api_host: '192.168.0.101',   // Standard networking term
    api_port: '8000',            // Keeps current naming
  },
  metadata: {
    ...
    async fetchAllData () {
      try {
        ...
        if (!useProxy) {
          ...
          apiBaseUrl = `http://${settings.api_host}:${settings.api_port}`
        }
        ...
        try {
          ...
          const { settings } = screenly
          const apiBaseUrl = `http://${settings.api_host}:${settings.api_port}`
